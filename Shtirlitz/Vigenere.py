...
class Vigenere:
    ...
    def caesar(self, letter, shift):
        if letter in self.alphaindex:  #шбжцлюэи ьтчоэ (строчная буква)
            index = (self.alphaindex[letter] + shift) % len(self.alphabet)
            cipherletter = self.alphabet[index]
        elif letter.lower() in self.alphaindex:  #йэряэоюэи ьтчоэ (заглавная буква)
            cipherletter = self.caesar(letter.lower(), shift).upper()
        else:
            cipherletter = letter
        return cipherletter

    def uncaesar(self, letter, shift):
        if letter in self.alphaindex:  #шбжцлюэи ьтчоэ (строчная буква)
            index = (self.alphaindex[letter] - shift) % len(self.alphabet)
            cipherletter = self.alphabet[index]
        elif letter.lower() in self.alphaindex:  #йэряэоюэи ьтчоэ (заглавная буква)
            cipherletter = self.uncaesar(letter.lower(), shift).upper()
        else:
            cipherletter = letter
        return cipherletter
    ...
    def decode(self, line):
        ...
        for i, letter in enumerate(line):
            shift = self.key[i % len(self.key)]
            cipherletter = self.uncaesar(letter, shift)
            ciphertext.append(cipherletter)

        return ''.join(ciphertext)


keyword = input('keyword = ') #столлман
